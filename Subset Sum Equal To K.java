...
public class Solution {
    public static boolean sub(int i,int target,int[] arr,int[][]dp){
        if(target==0)
        return true;
        if(i==0)
        return arr[i]==target;
        ...
        boolean ntaken = sub(i-1,target,arr,dp);
        boolean taken =false;
        if(target>=arr[i])
        taken = sub(i-1,target-arr[i],arr,dp);
        dp[i][target] = taken||ntaken?1:0;
        return taken||ntaken;
    }
