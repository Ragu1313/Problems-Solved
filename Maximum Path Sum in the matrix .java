...
public class Solution {
	public static int get(int i,int j,int n,int [][]mat ,int dp[][]){
		if(j<0 || j>=n){
		...
		int top = mat[i][j]+get(i-1,j,n,mat,dp);
		int rd = mat[i][j]+get(i-1,j+1,n,mat,dp);
		int ld = mat[i][j]+get(i-1,j-1,n,mat,dp);
		int res = Math.max(top,rd);
		res = Math.max(res,ld);
		return dp[i][j] = res;
	}
